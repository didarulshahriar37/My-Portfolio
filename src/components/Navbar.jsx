    ...
    return (
        <div
            className={`
                ...
                ${isScrolled
                    ? 'fixed top-0 left-0 w-full bg-white/95 dark:bg-gray-900/95 backdrop-blur-lg shadow-lg px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-24 border border-gray-700'
                    : 'relative w-11/12 mx-auto mt-5 bg-white/90 dark:bg-gray-900/80 backdrop-blur-md shadow-xl px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 rounded-2xl'
                }
            `}
        >
            <div className="flex h-14 sm:h-16 lg:h-20 items-center justify-between container mx-auto">
                ...
                <nav className="hidden md:flex items-center space-x-4 lg:space-x-6 xl:space-x-8">
                    {navLinks.map(link => (
                        <a key={link.text} href={link.href} className="text-sm lg:text-base font-medium text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors relative group">
                            {link.text}
            ...
            <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="py-4 border-t border-gray-200 dark:border-gray-800">
                    <div className="flex flex-col space-y-1">
                        {navLinks.map(link => (
                            <a key={link.text} href={link.href} onClick={() => setIsMenuOpen(false)} className="px-3 py-2.5 text-sm sm:text-base font-medium text-gray-600 dark:text-gray-300 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                                {link.text}
                            </a>
                        ))}
